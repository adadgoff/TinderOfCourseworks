...
class AuthRouter:
    ...
    async def login_student(
        ...
        student_login_read = await auth_service.login_student(
            student_login=student_login,
            session=self.session,
        )
        return student_login_read
    ...
    async def register_student(
        ...
        user = UserRegisterRead(email=student_register.email, password="placeholder")
        return user
    ...
    async def login_supervisor(
        ...
        supervisor_login_read = await auth_service.login_supervisor(
            supervisor_login=supervisor_login,
            session=self.session,
        )
        return supervisor_login_read
    ...
    async def register_supervisor(
        ...
        user = UserRegisterRead(email=supervisor_register.email, password="placeholder")
        return user
